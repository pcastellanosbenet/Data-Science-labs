...
app.layout = html.Div(children=[html.H1('SpaceX Launch Records Dashboard',
                                ...
                                html.Div(
                                    dcc.Dropdown(                                        
                                        id='site-dropdown',
                                        options=[
                                                {'label':'All Lanch Sites', 'value':'ALL'},
                                ...
                                dcc.RangeSlider(id='payload-slider',
                                                ...
                                                marks={m:str(m) for m in range(0, 11000, 2500)},
                                                value=[2500, 8000]
                                ),
...
def get_pie_chart(entered_site):    
    if entered_site == 'ALL':
        ...
        data = filtered_df[['Launch Site', 'class']].groupby('Launch Site').count().reset_index()
...
def get_scatter_plot(entered_site, payload_slider):
    if entered_site == 'ALL':
        ...
        filter = data['Payload Mass (kg)'].map(lambda v: True if min < v < max else False)
        data = data[filter]
        fig = px.scatter(data, x='Payload Mass (kg)', y='class',
                color = 'Booster Version Category',
